...
router.get("/",ensureAuthenticated, function(req, res){
    ...
    res.render("meeting/meetingforms", {name:name, user: user}); 
 });
    ...
    .then(meeting => {
        ...
        bcrypt.compare(req.body.meetingCode,meeting.meetingCode,(err,isMatch)=>{
            ...
            if(isMatch){
                ...
                meeting.save();
                res.redirect("/meeting/joinmeeting/" + id);
            } else {
...
router.get("/joinmeeting/:id",ensureAuthenticated, (req, res) => {
    meeting = Meeting.findOne({classroomId:req.params.id})
    res.render('meeting/meetingroom', {user: dcryptUser(req.user), vcroomId: req.params.id,room:meeting._id});
})
