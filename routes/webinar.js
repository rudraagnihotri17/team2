...
router.get("/",ensureAuthenticated, function(req, res){
    ...
    res.render("webinar/webinarforms", {name:name, user: user}); 
});
...
router.post("/joinwebinar",ensureAuthenticated, function(req, res){
    ...
    .then(webinar => {
        if(!webinar){
            ...
            return res.render("webinar/webinarforms",{errors,name:name, user: user});
        }
        var id = req.body.webinarId;
        // Match Password
        bcrypt.compare(req.body.webinarCode,webinar.webinarCode,(err,isMatch)=>{
            ...
            if(isMatch){
                ...
                webinar.save();
                res.redirect("/webinar/joinwebinar/" + id);
            } else {
                errors.push({ msg: 'Webinar Code is Wrong'});
                var user = dcryptUser(req.user);
                var name = user.fname + ' '+user.lname;
                res.render("webinar/webinarforms",{errors,name:name, user: user});
            }
        });
    })
    .catch(err => console.log(err));
...
router.get("/joinwebinar/:id",ensureAuthenticated, (req, res) => {
    webinar = Webinar.findOne({classroomId:req.params.id})
    res.render('webinar/webinarroom', {user: dcryptUser(req.user), vcroomId: req.params.id,room:webinar._id});
})
